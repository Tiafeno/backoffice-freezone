...
export class ImportArticleComponent implements OnInit {
  ...
  public loopColumn: Array<any> = [
    ...
    { key: 'ProductRef', label: "Réference du produit Freezone" },
    { key: 'SupplierRef', label: "Réference du fournisseur" },
    { key: 'Price', label: "Prix du fournisseur" },
    ...
    Papa.parse(this.Files, {
      ...
      step: async (results: any, parser: any) => {
        if (results || results.data) {
          ...
          if (_.isNull(this.Columns.ProductRef)) parser.resume();

          //let productsRef = this.Wordpress.products().sku(column[this.Columns.ProductRef]);
  ...
  createCategories(ctgs: any): Promise<Array<any>> {
